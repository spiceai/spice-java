...
`queryAsync()`/`queryAsyncWithParams()` submit a query for asynchronous execution and return an `AsyncQuery` handle instead of streaming results directly. This requires the Spice runtime to be running in distributed/scheduler mode; for the normal synchronous, streaming path use `query()`/`queryWithParams()`.
...
- Additive, backward-compatible change: every feature in this release is a new method; nothing existing was renamed, removed, or retyped.
- No new dependencies.
